...
export default function KnowledgePage({ accountId, groups, onError }: { accountId: string; groups: Group[]; onError: (value: string) => void }) {
  ...
  const remove = async () => {
    if (!currentBase || currentBase.readOnly || !window.confirm(`确认删除知识库“${currentBase.name}”？`)) return
    try {
      await api.deleteKnowledgeBase(accountId, currentBase.id)
      await reload()
    } catch (reason) {
      onError(readableError(reason))
    }
  ...
  return <div className="page-stack">
    <section className="section">
      <SectionHeading eyebrow="群专属资料" title="知识与 AI" meta={bases.length ? `${bases.length} 个知识库` : undefined} actions={<div className="button-row"><button className="secondary" onClick={() => void reload()}><RefreshCw size={15} />刷新</button><button className="primary" onClick={() => void create()}><Plus size={15} />新建知识库</button></div>} />
