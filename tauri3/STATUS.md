...
- `cargo test --no-default-features`：122 项（含 schema v1/v4/v5、损坏库原文件保护、ACK 事务回滚、序号阻塞、并发 claim、执行器排空、回执脱敏、数据库消息筛选分页、端口占用识别、生产能力校准、自动启动和关闭偏好校验）。
- `cargo test --features fixture`：123 项 + 2 项真实浏览器 CDP 集成测试，包含双群公告、双群全员禁言/解除、1000 条突发、101 条分批、完整 Runtime 副作用链和版本/脚本哈希校准。
- 两套 `cargo clippy --all-targets -- -D warnings` 通过。
- `pnpm test`：13 个文件、35 项 RTL/Vitest 通过，包含批量公告当前搜索结果全选、部分失败保留、未知回执人工确认、确认重启后等待 UAC 并续跑、规则完整字段、知识启停、审计筛选和消息游标分页回归测试。
