...
本文是本仓库的工程规范，也是**测试门禁的唯一权威来源**。其他文档一律链接到本文的“测试门禁”一节，不再各自维护命令清单。唯一例外是 [WINDOWS-ACCEPTANCE.md](WINDOWS-ACCEPTANCE.md) 第 4 节：它在离线 Windows 机器上被逐条执行，保留内联副本。
...
- AI Provider 必须由 `AiProviderPool` 创建，业务代码不得按消息新建 HTTP Client。连接、单连接生成和总预算分别为 2 秒、15 秒和 20 秒，主备采用顺序切换而非并发竞速。
